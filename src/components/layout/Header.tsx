    ...
    <header className="fixed top-20 left-0 w-full z-[100] bg-slate-950/90 backdrop-blur-xl border-b border-white/10 select-none shadow-2xl shadow-black/50">
      <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between gap-4">
        ...
        <nav className="hidden md:flex flex-1 relative overflow-hidden h-full items-center">
          ...
          >
            <div className="flex gap-3 px-8 items-center h-full">
              {CATEGORIES.map((cat) => {
            ...
            border transition-all duration-300 ease-out whitespace-nowrap
            overflow-hidden group active:scale-95 cursor-pointer
            
            ${
              isActive
                ? /* 1. 활성 상태: 애니메이션 제거. 배경/테두리/그림자로 '켜진' 상태만 표현 */
                  'bg-cyan-500/20 border-cyan-400 text-white shadow-[0_0_20px_-5px_rgba(6,182,212,0.5)]'
                : /* 2. 비활성 상태: 기존 유지 */
                  'bg-slate-900/50 border-cyan-500/30 text-slate-500 hover:text-cyan-400 hover:bg-cyan-500/10 hover:border-cyan-400/60 hover:shadow-[0_0_20px_-5px_rgba(6,182,212,0.4)]'
            }
          `}
                  >
                    {/* 텍스트 */}
          ...
          */}
                    <div
                      className={`
            absolute bottom-0 inset-x-0 h-[2px] bg-gradient-to-r from-transparent via-cyan-400 to-transparent blur-[1px] transition-opacity duration-300
            ${isActive ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}
          `}
                    />

                    {/* (삭제됨) 활성 상태 배경 스캔 애니메이션 div 제거 */}
                  </button>
                );
              })}
            </div>
                      ...
                      {BRAND_OPTIONS.map((brand) => {
                        ...
                        return (
                          ...
                          >
                            {brand}
                            {active && (
                              <div className="w-1 h-1 rounded-full bg-cyan-400 shadow-[0_0_5px_cyan]" />
                            )}
                          </button>
                        );
                      })}
        ...
        {isMenuOpen && (
          <>
            ...
            >
              ...
              <div className="mb-6">
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-500 mb-3 opacity-70">
                  Preferences
                </p>
              ...
              <div className="border-t border-white/10 pt-6">
                <div className="flex items-center justify-between mb-4">
                  <p className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-500 opacity-70">
                    Brands
                  </p>
