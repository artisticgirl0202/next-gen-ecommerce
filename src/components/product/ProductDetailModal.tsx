...
const allDemoProducts = (Array.isArray(demoProductsRaw) ? demoProductsRaw : []) as unknown as Product[];
...
export default function ProductDetailModal({
  ...
  const handleAdd = () => {
    ...
    feedbackTimeoutRef.current = window.setTimeout(() => setAddedFeedback(false), 1000);
  };
  ...
  useEffect(() => {
    ...
    const getFallbackRecommendations = (): Recommendation[] => {
      ...
      const currentCat = Array.isArray(product.categories) ? product.categories[0] : product.category;
    ...
    Promise.resolve().then(() => { if (mounted) setRecsLoading(true); });

    fetchHybridRecommendations(product.id, 6, controller.signal)
      .then((json) => {
        if (!mounted) return;
        let recs = Array.isArray(json?.recommendations) ? json.recommendations : Array.isArray(json) ? json : [];
  ...
  const getVal = (val?: string | string[]) => (Array.isArray(val) ? val.join(" / ") : val ?? "Information not specified");
      ...
      >
        ...
        >
            ...
            <p className="text-cyan-500/60 font-mono text-sm uppercase tracking-tighter">
                Category: {Array.isArray(product.categories) ? product.categories.join(" / ") : product.category || "General Module"}
            </p>
            </div>
