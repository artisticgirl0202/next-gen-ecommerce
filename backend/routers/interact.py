...
# from backend.services.cache import add_recent_click
...
# def interact(req: InteractRequest) -> dict[str, Any]:
#     """
#     예: POST /api/ai/
#     body: { user_id, product_id, event }
#     """
#     uid = str(req.user_id)
#     event = getattr(req, "event", "view")

#     # 1) Redis 실시간 반영 (view/click)
#     if event in ("view", "click"):
#         try:
#             add_recent_click(uid, req.product_id)
#         except Exception as e:
#             # 내부 에러가 있어도 사용자에게는 영향 주지 않음
#             # 필요하면 로거로 기록하도록 수정 가능
#             print("add_recent_click failed:", e)

#     # 2) Kafka (noop if disabled)
#     try:
#         send_user_event({
#             "user_id": req.user_id,
...
#         })
#     except Exception as e:
#         print("send_user_event failed:", e)

#     # 3) return
#     return {"ok": True}


# # backend/routers/interact.py (relevant excerpt)
# import inspect
# import logging
# logger = logging.getLogger(__name__)

# @router.post("/feedback", summary="AI feedback (order interaction)")
# async def ai_feedback(payload: InteractionCreate) -> dict[str, bool]:
#     """
#     프론트: POST /api/ai/feedback
#     body: { userId, orderId, action }
#     """
#     try:
#         # payload.user_id / payload.order_id are available (snake_case)
#         # If downstream expects ints, convert when possible
#         try:
#             uid = int(payload.user_id) if isinstance(payload.user_id, str) and payload.user_id.isdigit() else payload.user_id
#         except Exception:
#             uid = payload.user_id

#         try:
#             oid = int(payload.order_id) if isinstance(payload.order_id, str) and payload.order_id.isdigit() else payload.order_id
#         except Exception:
#             oid = payload.order_id

#         # call log_user_activity which may be sync or async
#         res = log_user_activity(user_id=uid, order_id=oid, action=payload.action)
#         if inspect.isawaitable(res):
#             await res
#     except Exception as e:
#         logger.exception("log_user_activity failed")
#         return {"ok": False}
